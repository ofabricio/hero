...
void Player::Update(float dt)
{
    ...
    MoveTo(mTargetPosition, dt * mMovementSpeed);
}
...
{
    if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
        SetTargetPosition(GetMouseTargetPosition());
    }
    if ((mCooldown += dt) >= 1.0f) {
        SetState(PlayerState::MOVE);
    }
...
void Player::SetTargetPosition(Vector3 target)
{
...
bool Player::MoveTo(Vector3 target, float speed)
{
